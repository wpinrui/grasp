 ...
 */
function sameReading(one: Written | null, other: Written | null): boolean {
...
const STOP_SLACK = 1.001;

function stopAbove(scale: number): number {
  return ZOOM_STOPS.find((stop) => stop > scale * STOP_SLACK) ?? MAX_SCALE;
}

function stopBelow(scale: number): number {
...
function stepped(from: Position, to: Position): Position {
...
function snapKey(snap: Snap | null): string {

/**
 * Putting a linked reading where its figure has got to.
 *
 ...
 * reading that has been linked holds its place in the frame of what it reads
...
export function readingsPlaced(objects: SketchObject[], settled: Settled): SketchObject[] {
  let moved = false;
  const next = objects.map((object) => {
    if (!isMeasurement(object) || !object.linked) return object;
    const frame = frameOf(object, objects, settled);
    if (!frame) return object;
    const at = spotIn(frame, object.linked);

...
/** Where the assets go, relative to the page that asks for them. */
const ASSETS = "assets";
...
/** The JSON island of that name, which is one string on its own line. */
function island<T>(bundle: string, name: string): T {
  ...
  const to = bundle.indexOf("</script>", from);
  return JSON.parse(bundle.slice(from, to).trim()) as T;
}
...
/**
 * The map from the page's own script, which reaches its two external files by
 * the addresses they came from rather than by substitution.
 */
function resourceScript(externals: External[], at: Record<string, string>): string {
  const map: Record<string, string> = {};
  for (const external of externals) {
    const path = at[external.uuid];
    if (path) map[external.id] = path;
  }

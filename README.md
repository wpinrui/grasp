...
- **Captions.** With nothing else selected, double-click a caption to switch to the Text tool and edit it. The palette has direct buttons for +, −, ×, ÷, fractions, ∠, °, ≤, ≥, <, and >, with more choices in Notation and Symbols. Stacked fractions grow to fit their numerator and denominator and reserve enough line spacing. Tab moves between notation slots. Click a measurement while writing a caption to insert a dynamic link, then continue typing after it. Click a link inside the caption to change its rounding, choose its unit, or hide the unit using the floating toolbar. These settings affect only that link. Each caption has one colour, set from the palette, shared by all its text, fractions, and measurement links.

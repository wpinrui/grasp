...
- **Captions.** Insert stacked fractions from Notation, with numerator and denominator widths that grow with the text and line spacing that accommodates the fraction. Tab moves between notation slots. Click a measurement while writing a caption to insert a dynamic link, then continue typing after it. Click a link inside the caption to change its rounding, choose its unit, or hide the unit using the floating toolbar. These settings affect only that link. Each caption has one colour, set from the palette, shared by all its text, fractions, and measurement links.
